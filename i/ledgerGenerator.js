 ...
 */
 async function getLedger(info) {
    const { customerType, customerNumber } = info;
    console.log("info Holo >>")
    ...
    try {
        ...
        const res = await universalGetRequest('getLedgerData', {
...
function renderLedgerTable(data, customerNo, customerType) {
    ...
    // টেবিল HTML জেনারেশন
    let html = `<h3>${customerType === 'member' ? 'সদস্য' : 'গ্রাহক'} নং: ${customerNo} এর বিস্তারিত লেজার</h3>`;
